#Purpose: Visualize leaf level timespace correlation models
#Author: LAS
...
#frequency
otf_list <- c(
  list.files("Script_output/Brms/time_spac_models/l.lvl/old/", 
...
otfplot80
#pull legend
tspalegend <- get_legend(otfplot80) #Why the FUCK wont you work?! 
otfplot80 <- otfplot80 +
...
ytf_list <- c(
  list.files("Script_output/Brms/time_spac_models/l.lvl/young/", 
             pattern = "^leaftotal.freq", 
             full.names = TRUE)
)
...
ytfplot80 <- ytfplot80 +
  ...
  scale_y_discrete (labels = c("Longitude"))+
  theme(legend.position = "none")
ytfplot80

#richness
otd_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/old/", 
...
otdplot80 <- otdplot80 +
  ...
  scale_y_discrete(labels = c("Latitude", "DOY", "Year"))+
  theme(legend.position = "none")
otdplot80

ytd_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/young/", 
                         pattern = "^leaftotal.rich", 
                         full.names = TRUE)
)
...
#Chewing data----
opac_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/old/", 
...
opacplot <- opacplot +
  ...
  scale_y_discrete(labels=c("DOY")) #keeping the legend becuase this is the only panel I'll save
opacplot

ypac_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/young/", 
                          pattern = "^leafperc_area_chew", 
                          full.names = TRUE)
)
...
#pres/abs chewing 
ochew_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/old/", 
                           pattern = "^chewvalue", 
                           full.names = TRUE)
)
...
ychew_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/young/", 
                           pattern = "^chewvalue", 
                           full.names = TRUE)
)
...
#combined chew plot----
# chewplot <- plot_grid(ypacplot,opacplot, #could change this to NULL but I also like having the empty plot so readers can see that there is no relationship. 
#                       ychewplot, ochewplot,
#                       nrow = 2,
#                       ncol = 2)
# chewplot
...
#Specialized data----
osf_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/old/", 
...
ysf_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/young/", 
                         pattern = "^leafspec.freq", 
                         full.names = TRUE)
)
...
osd_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/old/", 
...
osdplot <- osdplot +
  labs(title = "Richness of \nSpecialized Damage (late-season)")+
  scale_y_discrete(labels = c("Latitude", "Year"))+
  theme(legend.position = "none")
osdplot

ysd_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/young/", 
                         pattern = "^leafspec.rich", 
                         full.names = TRUE)
)
...
#Mining data----
opam_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/old/", 
...
ypam_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/young/", 
                          pattern = "^leafperc_area_mine", 
                          full.names = TRUE)
)
...
ypamplot 
ypamplot <- ypamplot +
  labs(title = "Percent Leaf \nArea Mined (early-season)") +
  scale_y_discrete(labels = c("DOY"))+
  theme(legend.position = "none")
ypamplot


omf_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/old/", 
...
ymf_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/young/", 
                         pattern = "^leafmine.freq", 
                         full.names = TRUE)
)
...
omd_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/old/", 
...
ymd_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/young/", 
                         pattern = "^leafmine.rich", 
                         full.names = TRUE)
)
...
mineplot <- plot_grid(ypamplot,opamplot, 
                      tspalegend,omdplot, #putting legend in the blank space
                      nrow = 2,
                      ncol = 2)
...
#Gall data----
opag_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/old/", 
...
###No data on young leaves for leaf percent galled?? Is that correct?? 
# ypag_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/young/", 
#                           pattern = "^leafperc_area_gall", 
#                           full.names = TRUE)
# )
...
ogf_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/old/", 
...
ygf_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/young/", 
                         pattern = "^leafgall.freq", 
                         full.names = TRUE)
)
...
ogd_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/old/", 
...
ygd_list <- c(list.files("Script_output/Brms/time_spac_models/l.lvl/young/", 
                         pattern = "^leafgall.rich", 
                         full.names = TRUE)
)
...
#combined Gall plot----
gallplot <- plot_grid( tspalegend,ogfplot, #using blank space for legend
                      ygdplot,ogdplot,
                      nrow = 2,
                      ncol = 2)
gallplot
...
#saving plots----
ggsave("figures/l.lvl/tspa.total_plot.pdf", totalplot80, height = 11, width = 8.5, units = "in")
ggsave("figures/l.lvl/tspa.chew_plot.pdf", opacplot, height = 11, width = 8.5, units = "in") #only saving opac plot because all others are empty and the figure looks stupid with only opac filled in 
ggsave("figures/l.lvl/tspa.spec_plot.pdf", specplot, height = 11, width = 8.5, units = "in")
ggsave("figures/l.lvl/tspa.mine_plot.pdf", mineplot, height = 11, width = 8.5, units = "in")
ggsave("figures/l.lvl/tspa.gall_plot.pdf", gallplot, height = 11, width = 8.5, units = "in")
