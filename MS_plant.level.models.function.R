#Purpose: Plant-level models to deal with non-independence of leaves
#Date started: 02.24.2025
#Author: LAS
#R version: 4.3.3.
...
simp_brm_models <- function(data, iter = 2000, cores = 2, init = 0, 
  ...
  for (response in response_vars) {
    ...
    for (model_type in names(formulas)) {
      ...
      write.csv(model,
                file = paste0("Script_output/Brms/simp_models/p.lvl/young/", model_name, "_model.csv"))
      
      # Write the model summary
      write.csv(summary(model)$fixed,
                file = paste0("Script_output/Brms/simp_models/p.lvl/young/", model_name, "_model_summary.csv"))
      
      #Bays R2 files
      write.csv(as.data.frame(r2),
                paste0("Script_output/Brms/simp_models/p.lvl/young/", model_name, "_bayes_R2.csv")) #here too too (added 02/13/2025)
    }
  }
...
create_brm_models <- function(data_file, response_data, additional_predictors = NULL, 
  ...
  # Function to determine the family based on the response variable name
  get_family <- function(response) {
    if (grepl("^leaf.*_area_", response) || grepl("^perc.", response) ||  grepl("chewvalue", response)) {
      return("zero_inflated_beta")
    } else if (grepl("\\.div$", response)) {
      return("gaussian")
    } else {
  ...
  model_types <- list(
    ...
    doy = function(response) paste(response, "~ scale(startDayOfYear)") #Add comma if interaction is needed
  ...
  for (response in response_vars) {
    ...
    for (predictor in all_predictors) {
      ...
      for (model_type_name in names(model_types)) {
        ...
        output_dir <- paste0("Script_output/Brms/climate_models/p.lvl/young/", model_type_name, "/")
...
oldYmodels <- create_brm_models("Script_output/PCA/p.lvl/old/top5oldY.csv", oldleaves.sum)

youngYmodels <- create_brm_models("Script_output/PCA/p.lvl/young/top5youngY.csv", youngleaves.sum)
