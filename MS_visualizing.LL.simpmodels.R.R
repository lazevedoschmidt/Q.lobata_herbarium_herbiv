#Purpose: Visualize Leaf Level model results for "simple" models
  #Model 1 = Herbivory ~ DOY
  #Model 2 = Herbivory ~ YearxDOY
  #Model 3 = Herbivory ~ Year
...
plot_forest_models <- function(csv_paths, model_names = NULL) {
  ...
  # Create the forest plot using all summaries (not just significant ones)
...
ypacplot80 <- ypacplot80 +
  labs(title = "Percent Leaf Area \nChewed (early-season)") +
  scale_y_discrete(labels=c("YearxDOY"))+ #we are replacing the y-axis (cord flip in function)
  theme(legend.position = "none") 
ypacplot80
...
specplot80 <- plot_grid(NULL,osfplot80, 
                        ysdplot80,osdplot80, 
                        nrow = 2,
                        ncol = 2)
...
ypam_names <- paste("Model", 1:1)
ypam_names <- factor(ypam_names, 
                     levels = paste("Model", 1:1),
                     ordered = TRUE)
...
# mineplot80 <- plot_grid(NULL,opamplot80, 
#                       nrow = 1,
#                       ncol = 2)
# mineplot80
...
gallplot80 <- plot_grid(NULL,ogfplot80,
                        ygdplot80,ogdplot80,
                        nrow = 2,
                        ncol = 2)
...
ggsave("figures/l.lvl/simp.specbrms80_plot.pdf", specplot80, height = 15, width = 13, units = "in")
ggsave("figures/l.lvl/simp.minebrms80_plot.pdf", opamplot80, height = 15, width = 13, units = "in")
#only opamplot80 was significant 
ggsave("figures/l.lvl/simp.gallbrms80_plot.pdf", gallplot80, height = 15, width = 13, units = "in")
